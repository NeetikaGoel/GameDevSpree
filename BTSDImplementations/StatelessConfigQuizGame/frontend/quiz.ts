...
//now functions to call backend and update frontend as per response

//so first of all if we got some attempt id or we need to make a new one
function quizAttemptIdGetFromUrl():string | null //might be str or null as per situation
{
    //we need to get whats the url parameters , using this function urlsearchparams is a built in function to get url parameters easily, now its parameter is window.location.search which will give us the url parameters part and then we can easily get quiz attempt id from it using get function (NEW INFO UNLOCKED HAHA)
    const quizUrlSearchParams=new URLSearchParams(window.location.search);
    const quizAttemptId=quizUrlSearchParams.get("quizAttemptId"); //this will give us the quiz attempt id if it is there in url parameters or else null

    if (!quizAttemptId || quizAttemptId.trim()==="") //TRIM IS USED TO REMOVE IF ANY EXTRA SPACE GOT IN
        {
            return null;
        }
    
    //NOW NO ISSUE, WE GOT ID, RETURN IT HURRAY
    return quizAttemptId;
}

//NOW WHAT IF NO ID, SO NEW ID NEED TO BE SET NOW
function quizAttemptIdSetInUrl(quizAttemptId:string):void
{
    const quizUrl=new URL(window.location.href); //GIVE NEW URL OBJ, BUILT IN FUNCTION, PARAMETER IS CURRENT URL
    quizUrl.searchParams.set("quizAttemptId",quizAttemptId); //THIS WILL SET THE QUIZ ATTEMPT ID IN URL PARAMETERS, FIRST PARAMETER IS KEY NAME AND SECOND IS VALUE
    window.history.replaceState({}, "", quizUrl.toString()); //NOW THIS WILL UPDATE URL AND WONT RELOAD PAGE WHICH IS BETTER BECOZ JUST UPDATE 
}
...
async function quizQuestionCurrentLoad():Promise<void> //RETURN PROMISE VOID BECOZ ASYNC FUNCTION ALWAYS RETURNS PROMISE AND WE DONT WANT TO RETURN ANY VALUE SO VOID, NOW PROMISE IS USED WHEN FUNC IS ASYNC
{
    ...
    const quizAttemptIdCurrent=quizAttemptIdGetFromUrl(); //FUNC USED WE DEFINED ABOVE

    let quizApiLoadUrlFinal=quizApiLoadUrl; //LET USED BECOZ MIGHT BE CHANGING IF ID IS THERE OR NOT, IF ID IS THERE THEN WE NEED TO ADD IT IN URL PARAMETERS

    if (quizAttemptIdCurrent) 
        {
            quizApiLoadUrlFinal=quizApiLoadUrl + "?quizAttemptId=" + encodeURIComponent(quizAttemptIdCurrent); //IF ID IS THERE THEN ADD IT IN URL PARAMETERS, ENCODE URI COMPONENT IS USED TO ENCODE THE ID IN CASE IT HAS SPECIAL CHARACTERS OR SPACES ETC, IT WILL MAKE SURE URL IS VALID, too tough to understand!!
        }

    const response=await fetch(quizApiLoadUrlFinal,{ 
        method:"GET" //wait for response from backend, method is get becoz we are just getting data from backend and not sending any data to backend, if we were sending data then it would be post
    });

 
    const quizResponseLoad:QuizResponseLoad=await response.json(); //now we have response in json to cnovert to obj and use built in function
