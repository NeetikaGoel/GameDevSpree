...
function quizSubmitAuthenticate(int $quizAttemptId,int $answerOptionIdByUser):bool
{
    return true; //PLACEHOLDER FOR NOW but 2 parameters coz maybe both used, will see
}

//AUTHORIZE FUNCTION
function quizSubmitAuthorize(int $quizAttemptId,int $answerOptionIdByUser):bool
{
    return true; //PLACEHOLDER FOR NOW
}

//RATE LIMIT FUNCTION
function quizSubmitRateLimitCheck(int $quizAttemptId,int $answerOptionIdByUser):bool
{
    return true; //PLACEHOLDER FOR NOW
}

//IDEMPOTENCY FUNCTION
function quizSubmitIdempotencyCheck(int $quizAttemptId,int $answerOptionIdByUser):bool
{
    return true; //PLACEHOLDER FOR NOW
}
...
function quizSubmitHandle():void
{
    ...
    if (isset($_POST['quizAttemptId']) && is_numeric((string)$_POST['quizAttemptId']))
        {
            $quizAttemptIdTemp=(int)$_POST['quizAttemptId'];
        }
    ...
    if (quizSubmitAuthenticate($quizAttemptIdTemp,$answerOptionIdByUserTemp)!==true)
        {
    ...
    /**
     2. AUTHORIZE
     */
    if (quizSubmitAuthorize($quizAttemptIdTemp,$answerOptionIdByUserTemp)!==true)
        {
    ...
    /**
     ...
     4. SANITIZE
     */
    if (!isset($_POST['quizAttemptId']) || !isset($_POST['answerOptionId']))
        {
            http_response_code(HTTP_STATUS_BAD_REQUEST);
            echo json_encode([
                'error'=>'quizAttemptId and answerOptionId are required!!'
            ]);
    ...
    $quizAttemptIdRaw=trim((string)$_POST['quizAttemptId']);
    $answerOptionIdByUserRaw=trim((string)$_POST['answerOptionId']);

    if ($quizAttemptIdRaw==='' || $answerOptionIdByUserRaw==='' || !is_numeric($quizAttemptIdRaw) ||!is_numeric($answerOptionIdByUserRaw))
        {
            http_response_code(HTTP_STATUS_BAD_REQUEST);
            echo json_encode([
    ...
    /**
     5. RATE LIMIT
     */
    if (quizSubmitRateLimitCheck($quizAttemptId,$answerOptionIdByUser)!==true)
        {
    ...
    /**
     6. IDEMPOTENCY
     */
    if (quizSubmitIdempotencyCheck($quizAttemptId,$answerOptionIdByUser)!==true)
        {
    ...
    $responseData=$quizSubmitService->quizSubmitService($quizAttemptId,$answerOptionIdByUser);
    ...
    /**
     9. AUDIT-LOG
     */
    quizSubmitAuditLog('quiz_submit_success',['quizAttemptId'=>$quizAttemptId,'answerOptionIdByUser'=>$answerOptionIdByUser,'isQuizDone'=>$responseData['isQuizDone']??false]);
}
