...
if (!isset($_SESSION['score']) || !isset($_SESSION['questionsDone']) || !isset($_SESSION['questionIdCurrent']) || !isset($_SESSION['questionIdOrder']) || !isset($_SESSION['questionIdOrderIndexCurrent']))
    {
...
//NEW CHECK INCOMING
if (!is_array($_SESSION['questionIdOrder']))
    {
        http_response_code(409);
        echo json_encode([
            'error' => 'Quiz session question order is invalid!!'
        ]);
        exit;
    }
...
//SPLITTING CHECKS COZ DIFF HTTP RESPONSE CODES
//checking if ans option id or ques id actually exists even or not
if ($answerOptionIdByUser<=0 || $questionIdByUser<=0)
    {
        http_response_code(400);
        echo json_encode([
            'error' => 'Question id and answer option id must be +ve nums!!'
        ]);
        exit;
    }

if ($answerOptionCurrent->getId()<=0 || $questionCurrent->getId()<=0)
    {
...
if ((int)$_SESSION['questionIdCurrent']!==$questionIdByUser)
    {
        http_response_code(409); //not found errors
        echo json_encode([
...
$_SESSION['questionIdOrderIndexCurrent']=(int)$_SESSION['questionIdOrderIndexCurrent']+1;
...
$questionIdOrderIndexNext = (int)$_SESSION['questionIdOrderIndexCurrent'];
...
if (!isset($questionIdOrder[$questionIdOrderIndexNext]))
    {
        //that means quiz has ended, all questions are done coz next idx is not in list
        echo json_encode([
...
$_SESSION['questionIdCurrent']=(int)$questionIdOrder[$questionIdOrderIndexNext];
$questionIdNext=(int)$_SESSION['questionIdCurrent'];
