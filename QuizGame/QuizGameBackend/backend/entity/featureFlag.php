...
function getFeatureFlag():bool
{
    ...
    //NEW CHECK INCOMING
    if (!isset($featureFlag[$featureFlagName]) || !is_array($featureFlag[$featureFlagName]) || !array_key_exists('enabled', $featureFlag[$featureFlagName]) //if there is no such field or if that field is not an array or if that field does not have the enabled key then we will return the default value
    )
        {
            return QUESTION_ORDER_RANDOM_ENABLED_DEFAULT;
        }
